...
const SearchManufacturer = ({ manufacturer, setManufacturer} : SearchManufacturerProps ) => {
    ...
    const filteredManufacturers = query === '' ? [] : manufacturers.filter((item)=> item.toLowerCase().replace(/\s+/g, '')
                  ...
                  afterLeave={() => setQuery('')} >
                    <Combobox.Options>
                      { /* filteredManufacturers.length === 0 && query !== '' ? (
                        <Combobox.Option value={query} className="search-manufacturer__option"> ***still function the same without it***
                          create {query}
                        </Combobox.Option>) : (*/ 
